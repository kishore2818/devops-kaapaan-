...
import { apiUrl, normalizeAssetUrl } from '../lib/api';
...
const Videos = () => {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl('/api/videos'))
      ...
      .then((res) => res.json())
      .then((data) => {
        setVideos(data.map((video) => ({ ...video, url: normalizeAssetUrl(video.url) })));
        setLoading(false);
      })
      .catch((err) => {
    ...
    // fetch(`https://kaapaan-backend.onrender.com/api/photos?videoName=${videoName}`)
      ...
      .then((data) => {
        if (data && Array.isArray(data)) {
          setRelatedPhotos(data.map((photo) => ({
            ...photo,
            imageUrl: normalizeAssetUrl(photo.imageUrl),
          })));
        }
      })
